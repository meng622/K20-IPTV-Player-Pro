...
class ChannelPanelManager:  # 面板管理器
    ...
    def init_channel_panel(self):  # 初始化頻道面板
        ...
        self.win.channel_panel.setMinimumWidth(450)  # 加寬整體頻道面板，提供舒適的寬度
        self.win.channel_panel.setMaximumWidth(1000)
    ...
    def switch_channel_view(self, mode, clicked_btn=None):
        ...
        if mode == "live":
            ...
            raw_list = getattr(self.win, data_list_name, []) # 保留：history_sources / local_media_list 有可能未初始化，用 getattr 畀預設空 list 好合理。
    ...
    def load_live_channels(self, reset=True):
        ...
        if isinstance(self.win.channels, list):
            self.win.channels = list({ch.get('url'): ch for ch in self.win.channels if isinstance(ch, dict) and ch.get('url')}.values())
        ...
        if reset or not self.win.filtered_channels_cache:
            self.win.channel_list.clear()
            ...
            if not all_data and not search_keyword:
                ...
                if not is_fetching:
                    ...
                    for worker_attr in ['m3u_worker', 'fetch_worker', 'm3u_fetch_worker']:
                        worker = getattr(self.win, worker_attr, None) # 保留：同樣，worker 屬性可能未存在，用 getattr 安全。
                        if worker and hasattr(worker, 'isRunning') and worker.isRunning(): #保留 hasattr：worker 可能係唔同類型，唔一定都有 isRunning，安全檢查。
                            is_fetching = True
                            break
    ...
    def toggle_ping_test(self):  # 開關測速
        ...
        if not self.win.local_media_list:
            return
        ...
        # 確保 latency dict 存在
        if self.win.channel_latencies is None:
    ...
    def on_channel_item_clicked(self, item):
        ...
        # 🎯 直接調用面板內部的 _get_current_epg_title 取得與左側卡片 100% 一致的 EPG
        current_epg = self._get_current_epg_title(ch) if isinstance(ch, dict) else ""
        if not current_epg and isinstance(ch, dict):
            current_epg = ch.get('epg', '') or ch.get('current_program', '')

        # ✅ 點擊時強制寫入全域頻道名稱，並印出 Log
        self.win.current_channel_name = clean_name
        print(f"📌 [DEBUG] 頻道已點擊，寫入 current_channel_name: '{clean_name}', EPG: '{current_epg}'")
        ...
        self.win.play_channel(item, custom_epg=current_epg, channel_name=clean_name)

    def _get_current_epg_title(self, ch):
        """極簡修復：完美複刻 epg_panel 的匹配邏輯，統一使用 clean_channel_name"""
        # 引入正確的清洗函數
        from epg_manager import clean_channel_name
        
        epg_cache = self.win.epg_data or self.win.epg_cache or {}
        if not epg_cache:
            return ""
        ...
        if isinstance(ch, dict):
            if ch.get('tvg_id'):
                keys_to_check.add(str(ch.get('tvg_id')).lower().strip())
                keys_to_check.add(clean_channel_name(ch.get('tvg_id')))
            if ch.get('tvg_name'):
                keys_to_check.add(str(ch.get('tvg_name')).lower().strip())
                keys_to_check.add(clean_channel_name(ch.get('tvg_name')))
            if ch.get('name'):
    ...
    def remove_history_or_local_item(self, ch, item, mode):
        ...
        target_list = getattr(self.win, attr_name, []) #保留：同上，攞 history/local 列表時安全。
    ...
    def load_epg_data(self, epg_urls_str):
        ...
        if urls:
            print("🚀 [DEBUG] 已轉交 main_window.load_epg 處理")
            self.win.load_epg(urls[0])
        else:
            print("❌ [DEBUG] 轉交失敗，找不到 self.win.load_epg")
